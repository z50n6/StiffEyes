...
// 统一的扫描过滤器
var StiffEyesScanFilter = {
  // API 过滤器
  api: (function() {
    return function(match, url, resultsSet) {
      match = match.slice(1, -1);
      if (StiffEyesScanConfig.API.FONT_PATTERN.test(match)) {
        return false;
      }
      ...
      if (StiffEyesScanConfig.API.IMAGE_PATTERN.test(match)) {
        resultsSet?.imageFiles?.set(match, url);
        return true;
      }
      if (StiffEyesScanConfig.API.JS_PATTERN.test(match)) {
        resultsSet?.jsFiles?.set(match, url);
        return true;
      }
      if (StiffEyesScanConfig.API.DOC_PATTERN.test(match)) {
        resultsSet?.docFiles?.set(match, url);
        return true;
      }
      const lcMatch = match.toLowerCase();
      const shouldFilter = StiffEyesScanConfig.API.FILTERED_CONTENT_TYPES.some(function (type) {
        return lcMatch.indexOf(type.toLowerCase()) >= 0;
      });
      if (shouldFilter) {
        return false;
      }
    ...
    const validate = {
      ...
      clean(domain) {
        try {
          ...
          // 4. 使用过滤规则提取域名
          const filterMatch = domain.match(StiffEyesScanConfig.PATTERNS.DOMAIN_FILTER);
      ...
      notInBlacklist(domain) {
        return !StiffEyesScanConfig.DOMAIN.BLACKLIST.some(blacklisted => 
  ...
  ip: (function() {
    const validate = {
      notSpecial(ip) {
        return !StiffEyesScanConfig.IP.SPECIAL_RANGES.some(range => range.test(ip));
      }
    };
    ...
    return function(match, url, resultsSet) {
      ...
      const ipMatch = match.match(StiffEyesScanConfig.PATTERNS.IP);
      if (ipMatch) {
  ...
  url: (match, url, resultsSet) => {
    try {
      ...
      if (matchUrl.host === currentHost) {
        ...
        if (StiffEyesScanConfig.API.FONT_PATTERN.test(path)) {
          return false;
        }
        if (StiffEyesScanConfig.API.IMAGE_PATTERN.test(path)) {
          resultsSet?.imageFiles?.set(path, url);
          return true;
        }
        if (StiffEyesScanConfig.API.JS_PATTERN.test(path)) {
          resultsSet?.jsFiles?.set(path, url);
          return true;
        }
        if (StiffEyesScanConfig.API.DOC_PATTERN.test(path)) {
  ...
  company: (match, url, resultsSet) => {
    ...
    if (Array.from(StiffEyesScanConfig.BLACKLIST.CHINESE_BLACKLIST).some(blackWord=>match.includes(blackWord))) return false;
  ...
  cookie: (match, url, resultsSet) => {
    ...
    if (value.length<12){
      if(Array.from(StiffEyesScanConfig.BLACKLIST.SHORT_VALUES).some(blackWord=>value.includes(blackWord))){
        return false;
      }
    }else{
      if(Array.from(StiffEyesScanConfig.BLACKLIST.MEDIUM_VALUES).some(blackWord=>value.includes(blackWord))){
        return false;
      }
    }
  ...
  id_key: (match, url, resultsSet) => {
    ...
    if (hasDelimiter || match.length >= 32) {
      ...
      if (hasDelimiter) {
        ...
        // 检查key是否在黑名单中
        if(Array.from(StiffEyesScanConfig.ID_KEY.KEY_BLACKLIST).some(blackWord=>keyLower.includes(blackWord))){
          return false;
        }
        // 检查value是否在统一黑名单中
        if(value.length<16){
          if(Array.from(StiffEyesScanConfig.BLACKLIST.SHORT_VALUES).some(blackWord=>valueLower.includes(blackWord))){
            return false;
          }
          if(Array.from(StiffEyesScanConfig.BLACKLIST.MEDIUM_VALUES).some(blackWord=>valueLower.includes(blackWord))){
            return false;
          }
        }else{
          if(Array.from(StiffEyesScanConfig.BLACKLIST.MEDIUM_VALUES).some(blackWord=>valueLower.includes(blackWord))){
          ...
          if(Array.from(StiffEyesScanConfig.BLACKLIST.LONG_VALUES).some(blackWord=>valueLower.includes(blackWord))){
            return false;
          }
        }
        // 其他检查
        ...
        // 检查value是否在统一黑名单中
        if(Array.from(StiffEyesScanConfig.BLACKLIST.MEDIUM_VALUES).some(blackWord=>match.includes(blackWord))){
          return false;
        }
        if(Array.from(StiffEyesScanConfig.BLACKLIST.LONG_VALUES).some(blackWord=>match.includes(blackWord))){
          return false;
        }
      }
