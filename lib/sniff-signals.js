...
// Pass 1 (Header): from response headers
// Pass 2 (DOM): from page content
(function () {
  'use strict';

  function arr(value) { return Array.isArray(value) ? value : (value === undefined || value === null ? [] : [value]); }

  // Build header-only signal input (from response headers)
  ...
  // Build DOM-only signal input (from page content)
