var StiffEyesScanConfig = {
  // API 相关配置
  API: {
    PATTERN: /['"`](?:\/|\.\.\/|\.\/)[^\/\>\< \)\(\}\,\'\"\\](?:[^\^\>\< \)\(\,\'\"\\])*?['"`]|['"`][a-zA-Z0-9]+(?<!text|application)\/(?:[^\^\>\< \)\(\{\}\,\'\"\\])*?["'`]/g,
    // 图片文件模式
...
// 域名匹配 - HTML页面
    ...
    get API() {
      return StiffEyesScanConfig.API.PATTERN;
    },
    ...
    get CREDENTIALS() {
      return {
        type: 'CREDENTIALS',
        patterns: StiffEyesScanConfig.CREDENTIALS.PATTERNS
      };
    },
    ...
    get ID_KEY() {
      return {
        type: 'ID_KEY',
        patterns: StiffEyesScanConfig.ID_KEY.PATTERNS
      };
    },
    get FINGER() {
      return {
        type: 'FINGER',
        patterns: StiffEyesScanConfig.FINGER.PATTERNS
      };
    }
  },
  // 统一的黑名单配置
