...
const SCANNER_CONFIG = {
  ...
  SECRETS: {
    PATTERNS: [
      // Webhook URL
      {name: 'Slack Webhook', pattern: /https:\/\/hooks\.slack\.com\/services\/[A-Za-z0-9\/_+]{10,}/gi},
      {name: 'Discord Webhook', pattern: /https:\/\/discord(?:app)?\.com\/api\/webhooks\/\d+\/[A-Za-z0-9_-]+/gi},
      {name: '钉钉 Webhook', pattern: /https:\/\/oapi\.dingtalk\.com\/robot\/send\?access_token=[A-Za-z0-9]+/gi},
      {name: '飞书 Webhook', pattern: /https:\/\/open\.feishu\.cn\/open-apis\/bot\/v2\/hook\/[A-Za-z0-9-]+/gi},
      // Private keys
      {name: 'RSA 私钥', pattern: /-----BEGIN RSA PRIVATE KEY-----/gi},
      {name: 'OPENSSH 私钥', pattern: /-----BEGIN OPENSSH PRIVATE KEY-----/gi},
      {name: '通用私钥', pattern: /-----BEGIN (?:EC |DSA |PGP )?PRIVATE KEY-----/gi},
      // Cloud service keys (named key = value pattern)
      {name: '云服务密钥', pattern: /(?:access_?key|access_?token|access_?secret|api[_-]?key|api[_-]?secret|api[_-]?token|app[_-]?key|app[_-]?secret|app[_-]?token|auth_?token|aws_?(?:access_?key|secret|bucket|token)|client_?secret|consumer_?key|consumer_?secret|db_?(?:pass|password|user|username)|encryption_?key|private_?key|secret_?(?:key|token|access)|bucket_?(?:name|password)|cloudflare_?(?:api_?key|auth_?key)|datadog_?(?:api_?key|app_?key)|docker_?(?:hub_?password|pass|passwd|password|key)|firebase_?api_?key|github_?(?:token|secret|key)|gitlab_?(?:token|secret)|heroku_?api_?key|jwt_?secret|mailgun_?api_?key|mapbox_?(?:access_?token|secret)|paypal_?(?:client_?id|secret)|redis_?(?:url|password|auth)|sendgrid_?api_?key|sentry_?(?:auth_?token|dsn)|slack_?(?:api_?token|webhook)|sonar_?(?:token|login)|stripe_?(?:secret|key)|twilio_?(?:account_?sid|auth_?token|api_?key))[a-z0-9_ .\-]{0,25}\s*[:=]\s*["'][^"'\s]{6,}["']/gi},
      // Database connection strings
      {name: '数据库连接串', pattern: /(?:jdbc|mysql|postgres(?:ql)?|mongodb|redis|sqlserver|oracle|mssql):\/\/(?!localhost|127\.0\.0\.1)[^\/\s]{1,}[^\s'")]{0,80}/gi},
      // Payment keys
      {name: 'Stripe 密钥', pattern: /(?:sk|pk)_(?:live|test)_[A-Za-z0-9]{24,}/g},
      {name: '支付宝密钥', pattern: /(?:AKLT|AKTP)[a-zA-Z0-9]{35,50}/g},
      // Cloud platform AK
      {name: '百度云 AK', pattern: /AK[A-Za-z0-9]{14,40}/g},
      // 金山云 AK (16-28 chars, distinct from 支付宝 35-50)
      {name: '金山云 AK', pattern: /\bAKLT[a-zA-Z0-9-_]{16,28}\b/g},
      // Access / auth tokens
      {name: 'Access Token', pattern: /(?:access_?token|auth_?token|api_?token|bearer_?token|refresh_?token)\s*[:=]\s*["']?[A-Za-z0-9\-_.~+\/=]{10,}["']?/gi},
      // Generic token in assignment context
      {name: 'Token 密钥', pattern: /\b(?:token|secretToken|appToken)\s*[:=]\s*["'][A-Za-z0-9\-_.]{8,}["']/gi},
      // CTF flag
      {name: 'CTF Flag', pattern: /flag\{[a-zA-Z0-9_\-!@#$%^&*()+=\[\]{}|;:,.<>?\/~`]+\}/gi},
      // Swagger/API docs
      {name: 'Swagger UI', pattern: /(?:swagger-ui\.html|swagger\.json|swagger\.yaml|api-docs|\/v2\/api-docs|\/v3\/api-docs|swaggerUi|swaggerVersion)/gi},
      // Sensitive admin paths
      {name: '敏感管理路径', pattern: /["']\/(?:admin|manage|manager|dashboard|console|actuator|druid)\/[^\s"']*["']/gi},
      // SMTP credentials
      {name: 'SMTP 配置', pattern: /(?:SMTP|smtp)_(?:HOST|PORT|USER|PASS(?:WORD)?)\s*[:=]\s*["'][^"']{2,}["']/gi},
      // URL redirect parameters
      {name: 'URL 跳转参数', pattern: /[?&](?:redirect_?url|redirect_?to|jump_?to|return_?url|callback_?url|target_?url|redir|goto|next)=https?:\/\//gi},
      // Bank card
      {name: '银行卡号', pattern: /\b[1-9]\d{15,18}\b/g},
      // Car plate
      {name: '车牌号', pattern: /[京津沪渝冀豫云辽黑湘皖鲁新苏浙赣鄂桂甘晋蒙陕吉闽贵粤青藏川宁琼使领][A-HJ-NP-Z][A-HJ-NP-Z0-9]{4,5}[A-HJ-NP-Z0-9挂学警港澳]/g}
    ]
  },
