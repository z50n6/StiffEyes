...
var StiffEyesJwt = (function () {
  ...
  /** 返回算法类别：hmac | rsa | ec | pss */
  function algCategory(alg) {
    if (!alg) return '';
    if (alg.indexOf('HS') === 0) return 'hmac';
    if (alg.indexOf('RS') === 0) return 'rsa';
    if (alg.indexOf('ES') === 0) return 'ec';
    if (alg.indexOf('PS') === 0) return 'pss';
    return '';
  }
  ...
  /** 验证 JWT 签名。返回 { valid, error } */
  async function verifyToken(token, secret, alg) {
    if (!token || !token.trim()) return { valid: false, error: '无令牌' };
    var api = joseApi();
    try {
      var cat = algCategory(alg);
      if (cat === 'hmac') {
        if (!secret) return { valid: false, error: '缺少 HMAC 密钥' };
        await api.jwtVerify(token, new TextEncoder().encode(secret));
      } else if (cat === 'rsa' || cat === 'ec' || cat === 'pss') {
        if (!keyPair.pub) return { valid: false, error: '缺少公钥' };
        var pubKey = await api.importJWK(keyPair.pub, alg);
        await api.jwtVerify(token, pubKey);
      } else {
        return { valid: false, error: '未知算法类别' };
      }
  ...
  return {
    ...
    getAlgCategory: function (alg) {
      return algCategory(alg);
    },
