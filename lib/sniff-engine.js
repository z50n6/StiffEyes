// StiffEyes sniff engine — two-pass detection engine
...
// Results merged via mergeFingerprintHitResults (same as two-pass design)
(function () {
  'use strict';
  ...
  // Two-pass detection using two-pass design:
  ...
  function analyze(compiledStore, headerInput, domInput) {
    ...
    // Pass 1: Header-only signals (from response headers)
    var headerHits = detectPass(compiledStore, headerInput || {});

    // Pass 2: DOM-only signals (from page DOM)
    var domHits = detectPass(compiledStore, domInput || {});

    // Merge both pass results (using standard merge)
    var merged = [];
